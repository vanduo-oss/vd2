...
const icons: Record<string, string> = { system: "ph-desktop", light: "ph-sun", dark: "ph-moon" };
...
const cssApi: [string, string, string][] = [
  ['data-theme-ui="menu"', "On .vd-theme-switcher — enables icon menu variant (recommended for navbar).", "Attribute"],
  [".vd-theme-switcher-toggle", "Menu toggle button; icon synced via [data-theme-icon].", "Class"],
  ["[data-theme-value]", "Menu option value: system, light, or dark.", "Attribute"],
  ["data-tooltip", "Optional tooltips on toggle and menu options (requires Tooltips component).", "Attribute"],
  ['data-toggle="theme"', "Required attribute on button or select element to bind theme switching behavior.", "Attribute"],
  ['data-theme="light|dark"', "Applied to <html> element. Controls CSS variable overrides for dark/light mode. In system preference, this is set to the resolved light/dark value.", "State Attribute"],
  [".theme-current-label", "Class for span inside button to auto-update with current theme label (System, Light, Dark).", "Utility Class"],
  ["data-theme-initialized", "Added to elements after ThemeSwitcher binds events. Used for testing and debugging.", "State Attribute"],
];
</script>
  ...
  <DocsLayout>
    <section id="theme-switcher">
      ...
      <p class="vd-mb-8">
        ...
        dark, and system color modes. Unlike the full Theme Customizer, it
        focuses solely on the color mode toggle without additional customization
        options. Perfect for when you need a simple dark mode button.
      </p>
      ...
      <div class="vd-card vd-card-glow demo-card vd-mb-8">
        ...
        <div class="vd-card-body">
          <p class="vd-mb-5">Try both variants — icon menu for navbars, cycle button for one-click toggling:</p>
          <div id="theme-switcher-demo" class="vd-row vd-mb-5">
            <div class="vd-col-12 vd-col-md-6">
              <p class="vd-text-sm vd-font-semibold vd-text-center vd-mb-3">Icon Menu</p>
              <div class="theme-switcher-demo-strip">
                <div class="theme-switcher-demo-slot">
                  <div class="vd-theme-switcher vd-theme-switcher-menu-end" :class="{ 'is-open': menuOpen }" data-theme-ui="menu">
                    <button type="button" class="vd-theme-switcher-toggle" :aria-label="`Theme: ${current}`" @click="menuOpen = !menuOpen">
                      <i class="ph" :class="currentIcon" data-theme-icon aria-hidden="true"></i>
                    </button>
                    <div class="vd-theme-switcher-menu" role="menu" :aria-hidden="!menuOpen">
                      <button v-for="opt in cycle" :key="opt" type="button" role="menuitemradio" :data-theme-value="opt" :aria-checked="current === opt" @click="pick(opt)">
            ...
            <div class="vd-col-12 vd-col-md-6">
              <p class="vd-text-sm vd-font-semibold vd-text-center vd-mb-3">Cycle Button</p>
              <div class="theme-switcher-demo-strip">
                <div class="theme-switcher-demo-slot">
                  <button type="button" class="vd-theme-switcher-toggle" :aria-label="`Theme: ${current}`" @click="cycleTheme">
                    <i class="ph" :class="currentIcon" data-theme-icon aria-hidden="true"></i>
                  </button>
                </div>
              </div>
          ...
          <p class="vd-text-sm vd-text-muted vd-text-center">Current theme: <span class="vd-font-semibold">{{ current }}</span></p>
        </div>
      </div>
      ...
      <div class="vd-row">
        <div class="vd-col-12 vd-col-md-6">
          <div class="vd-card vd-card-glow demo-card">
            ...
            <div class="vd-card-body">
              <p>Use the menu variant for navbars — toggle opens an icon-only picker without changing theme until the user chooses:</p>
              <DocCodeSnippet :html="menuCodeHtml" />
              <p class="vd-mt-5"><strong>Try it:</strong> Use the theme switcher in the navbar or the live demo above.</p>
            </div>
          </div>
        </div>
        <div class="vd-col-12 vd-col-md-6">
          <div class="vd-card vd-card-glow demo-card">
            ...
            <div class="vd-card-body">
              <p>Add <code>data-toggle="theme"</code> to cycle System → Light → Dark on each click:</p>
              <DocCodeSnippet :html="cycleCodeHtml" />
              <p class="vd-mt-5"><strong>Note:</strong> Cycle buttons change theme immediately on each click — best for minimal one-click apps, not navbars where accidental flashes matter.</p>
            </div>
          </div>
        </div>
        <div class="vd-col-12 vd-col-md-6">
      ...
      <div class="vd-row">
        <div class="vd-col-12 vd-col-md-6">
          <div class="vd-card vd-card-glow demo-card">
            <div class="vd-card-header"><h6>ThemeSwitcher vs ThemeCustomizer</h6></div>
            <div class="vd-card-body">
              <div class="vd-table-responsive">
                <table class="vd-table vd-table-hover">
                  <thead><tr><th>Feature</th><th>ThemeSwitcher</th><th>ThemeCustomizer</th></tr></thead>
                  <tbody>
                    <tr v-for="row in compareRows" :key="row[0]">
                      <td>{{ row[0] }}</td>
                      <td>
                        <template v-if="row[1] === true"><i class="ph ph-check vd-text-success"></i></template>
                        <template v-else-if="row[1] === false"><i class="ph ph-x vd-text-muted"></i></template>
                        <template v-else><i class="ph ph-x vd-text-muted"></i> {{ row[1] }}</template>
                      </td>
                      <td>
                        <template v-if="row[2] === true"><i class="ph ph-check vd-text-success"></i></template>
                        <template v-else-if="row[2] === false"><i class="ph ph-x vd-text-muted"></i></template>
                        <template v-else-if="typeof row[2] === 'string' && row[2].startsWith('(')"><i class="ph ph-check vd-text-success"></i> {{ row[2] }}</template>
        ...
        <div class="vd-col-12 vd-col-md-6">
          <div class="vd-card vd-card-glow demo-card">
            ...
            <div class="vd-card-body">
              <p>When using ThemeCustomizer with ThemeSwitcher, you can define different primary colors for light and dark modes:</p>
              <DocCodeSnippet :js="perThemeJs" />
              <p class="vd-mt-5 vd-text-sm vd-text-muted">When the user switches themes, the primary color automatically changes if they're using the default. If they've manually selected a color, it stays unchanged.</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="vd-card-body">
              <div class="vd-table-responsive">
                <table class="vd-table vd-table-striped">
                  <thead><tr><th>Key</th><th>Default</th><th>Values</th></tr></thead>
                  <tbody>
                    <tr><td><code>vanduo-theme-preference</code></td><td><code>system</code></td><td>system, light, dark</td></tr>
                  </tbody>
                </table>
              </div>
              <p class="vd-mt-5 vd-text-sm vd-text-muted">The preference is automatically saved and restored on page load.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="vd-table-responsive" style="margin-bottom: 3rem">
        <table class="vd-table vd-table-hover">
          <thead>
            <tr><th style="width: 25%">Attribute</th><th style="width: 55%">Description</th><th style="width: 20%">Type</th></tr>
          </thead>
          <tbody>
