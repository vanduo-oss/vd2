...
const cssClasses: [string, string][] = [
  ...
  [".vd-rating-readonly", "Added to the container when read-only; disables hover/click interactions"],
  [".vd-rating-sm", "Small size variant — place on the trigger element alongside data-vd-rating"],
...
const dataAttrs: [string, string][] = [
  ...
  ["data-vd-rating-readonly", "Makes the rating non-interactive (display only)"],
];

const jsMethods: [string, string][] = [
  ["VanduoRating.init()", "Scans for all [data-vd-rating] elements and renders star widgets"],
  ...
  <DocsLayout>
    <section id="rating">
            ...
            <table class="vd-table vd-table-striped">
              ...
              <tbody>
                <tr>
                  ...
                  <td>Fired on the element when the user selects a new value. <code>event.detail</code> contains <code>{ value: number, max: number }</code></td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="vd-card vd-card-glow demo-card">
        ...
        <div class="vd-card-body">
          <ul>
            <li>The container has <code>role="radiogroup"</code> with an <code>aria-label</code> like "Rating: 3 out of 5"</li>
            <li>Each star has <code>role="radio"</code> with <code>aria-checked</code> and <code>aria-label</code> (e.g. "3 stars")</li>
            <li><kbd>←</kbd> / <kbd>→</kbd> arrow keys change the value; <kbd>Home</kbd> sets to 1; <kbd>End</kbd> sets to max</li>
            <li>Read-only ratings use <code>aria-readonly="true"</code> and are not focusable</li>
            <li>The widget is fully operable without a mouse via <kbd>Tab</kbd> + arrow keys</li>
          </ul>
        </div>
      </div>
    </section>
