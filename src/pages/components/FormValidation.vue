...
const jsMethods: [string, string][] = [
  ["VanduoValidate.init()", "Scans the DOM for [data-vd-validate] forms and attaches validation"],
  ["VanduoValidate.validateForm(form)", "Programmatically validates all fields in the given <form> element. Returns true if valid"],
  ["VanduoValidate.addRule(name, validator, message)", "Register a custom rule. validator(value, param) must return true or false"],
];
...
<template>
  <DocsLayout>
    <section id="form-validation" ref="root">
      <h5 class="demo-title"><i class="ph ph-shield-check"></i>Form Validation</h5>
      <p class="vd-mb-5">
      ...
      <div class="vd-row">
            ...
            <div class="vd-card-body">
              <form data-vd-validate novalidate @submit="noop">
                <div class="vd-form-group">
                  ...
                  <input type="text" id="val-name" class="vd-input" placeholder="Jane Doe"
                    data-vd-rules="required" data-vd-msg-required="Please enter your name" />
                </div>
                <div class="vd-form-group">
                  <label for="val-email">Email Address</label>
                  <input type="email" id="val-email" class="vd-input" placeholder="jane@example.com"
                    data-vd-rules="required|email" />
                </div>
                <button type="submit" class="vd-btn vd-btn-primary">Submit</button>
              </form>
        ...
        <div class="vd-col-12 vd-col-md-6">
          <div id="demo-validate-length" class="vd-card vd-card-glow demo-card">
            <div class="vd-card-header"><h6>Min / Max Length &amp; Number Range</h6></div>
            <div class="vd-card-body">
              <form data-vd-validate novalidate @submit="noop">
                <div class="vd-form-group">
                  <label for="val-username">Username (3–20 chars)</label>
                  <input type="text" id="val-username" class="vd-input" placeholder="vanduo_user"
                    data-vd-rules="required|min:3|max:20" />
                </div>
                <div class="vd-form-group">
                  <label for="val-age">Age (18–120)</label>
                  <input type="number" id="val-age" class="vd-input" placeholder="25"
                    data-vd-rules="required|number|minVal:18|maxVal:120" />
                </div>
                <button type="submit" class="vd-btn vd-btn-primary">Submit</button>
              </form>
      ...
      <div class="vd-row">
            ...
            <div class="vd-card-body">
              <form data-vd-validate novalidate @submit="noop">
                <div class="vd-form-group">
                  ...
                  <input type="password" id="val-pass" class="vd-input" placeholder="••••••••"
                    data-vd-rules="required|min:8" />
                </div>
                <div class="vd-form-group">
                  <label for="val-pass-confirm">Confirm Password</label>
                  <input type="password" id="val-pass-confirm" class="vd-input" placeholder="••••••••"
                    data-vd-rules="required|match:val-pass" data-vd-msg-match="Passwords do not match" />
                </div>
                <button type="submit" class="vd-btn vd-btn-primary">Create Account</button>
              </form>
        ...
        <div class="vd-col-12 vd-col-md-6">
          <div id="demo-validate-modes" class="vd-card vd-card-glow demo-card">
            <div class="vd-card-header"><h6>Validation Modes &amp; Custom Messages</h6></div>
            <div class="vd-card-body">
              <form data-vd-validate data-vd-validate-mode="blur" novalidate @submit="noop">
                <div class="vd-form-group">
                  <label for="val-website">Website (validate on blur)</label>
                  <input type="text" id="val-website" class="vd-input" placeholder="https://example.com"
                    data-vd-rules="required|url"
                ...
                <div class="vd-form-group">
                  ...
                  <input type="text" id="val-pattern" class="vd-input" placeholder="#ff6600"
                    data-vd-rules="required|pattern:^#[0-9a-fA-F]{6}$"
                    data-vd-validate-mode="input"
      ...
      <div class="vd-card vd-card-glow demo-card">
        <div class="vd-card-header">
          <h6><i class="ph ph-list-checks mr-2" style="color: var(--vd-color-primary);"></i>Built-in Validation Rules</h6>
        </div>
        <div class="vd-card-body">
          <div class="vd-table-responsive">
            <table class="vd-table vd-table-hover">
              <thead><tr><th>Rule</th><th>Syntax</th><th>Description</th></tr></thead>
              <tbody>
            ...
            <table class="vd-table vd-table-hover">
              ...
              <tbody>
                <tr>
                  ...
                  <td>Fired on the form when all fields pass validation on submit. <code>event.detail</code> contains form data as a plain object</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="vd-card vd-card-glow demo-card">
        ...
        <div class="vd-card-body">
          <ul>
            <li>Invalid fields receive <code>aria-invalid="true"</code>; valid fields get <code>aria-invalid="false"</code></li>
            <li>Error messages are linked via <code>aria-describedby</code> so screen readers announce them</li>
            <li>Error elements use <code>role="alert"</code> for immediate announcement on live changes</li>
            <li>Native <code>&lt;form&gt;</code> semantics preserved — <code>novalidate</code> disables browser UI, not semantics</li>
            <li>Focus moves to the first invalid field on submit for keyboard users</li>
          </ul>
        </div>
      </div>
    </section>
