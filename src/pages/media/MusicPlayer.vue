...
interface MusicPlayerApi {
  ...
  setPosition: (el: HTMLElement, position: string | { x: number; y: number }) => void;
...
type ComponentExpose = { player: MusicPlayerApi; container: () => HTMLElement | null };
...
const variantDemos: VariantDemo[] = [
  ...
  },
  {
    ...
    blurb: "Enable showPlaylist and showProgress for a fuller player. Shuffle and repeat are always on the control bar.",
  ...
  },
  {
    ...
    blurb: "Standard layout with track name — use vd-music-player-sm or omit size classes.",
  ...
  },
  {
    key: "lg",
    title: "Large",
    blurb: "Roomier padding and controls — vd-music-player-lg for featured placement.",
...
const programmaticApi: [string, string][] = [
  ...
  ["shuffle(el) / repeat(el) / setRepeat(el, 'one')", "Shuffle toggle; cycle/set repeat mode."],
  ["detach(el, 'bottom-right') / attach(el)", "Float above the page / dock back (requires detachable)."],
  ["setPosition(el, 'top-right') / setPosition(el, { x, y })", "Reposition a detached player."],
  ["minimize(el) / expand(el) / toggleMinimize(el)", "Collapse/restore (requires minimizable)."],
  ["getState(el)", "Returns { isPlaying, currentIndex, currentTrack, volume, shuffle, repeat, tracks, isDetached, isMinimized }."],
  ...
  <DocsLayout>
    <section id="music-player">
      ...
      <p class="vd-mb-4">
        ...
        volume, optional shuffle and repeat modes, seek bar, playlist, glass surface,
        and a detachable floating mode (corner placement, pointer-drag reposition,
        minimizable). Zero external dependencies, powered by the native Audio API,
        and it automatically adapts to the active Vanduo theme.
      </p>
      <p class="vd-text-sm vd-text-muted vd-mb-8">
        Install the player separately from the core framework. All live demos below
        use a bundled sample playlist from <em>Invent the Universe</em>, with
        <em>Pale Blue Dot</em> as the default first track. Ships an optional Vue 3
        binding (<code>@vanduo-oss/music-player/vue</code>) used here.
      </p>

      <!-- Minimal + progress -->
      <div class="vd-row vd-mb-6">
        <div v-for="d in variantDemos.slice(0, 2)" :key="d.key" class="vd-col-12 vd-col-md-6 vd-mb-6">
        ...
        <div class="vd-col-12 vd-mb-6">
          <div class="vd-card vd-card-glow demo-card">
            <div class="vd-card-header">
              ...
              <p class="vd-text-sm vd-text-muted vd-mb-0">{{ variantDemos[2].blurb }}</p>
            </div>
            <div class="vd-card-body">
        ...
        <div class="vd-col-12 vd-mb-6">
          <div class="vd-card vd-card-glow demo-card">
            <div class="vd-card-header">
              ...
              <p class="vd-text-sm vd-text-muted vd-mb-0">{{ variantDemos[6].blurb }}</p>
            </div>
            <div class="vd-card-body">
              <VdMusicPlayer
                :tracks="tracks"
          ...
          <div class="vd-card vd-card-glow demo-card">
            <div class="vd-card-header">
              ...
              <p class="vd-text-sm vd-text-muted vd-mb-0">
                Add <code>glass: true</code> or the <code>vd-music-player-glass</code> class to any player size or layout. Uses the same frosted tokens as <code>vd-glass</code> in the core framework.
              </p>
            </div>
            <div class="vd-card-body">
              <div
                ...
                style="
                  ...
                  background: linear-gradient(120deg, #ff6b6b 0%, #4ecdc4 50%, #45b7d1 100%);
                  position: relative;
                  overflow: hidden;
                "
              >
                <div
                  style="
                    ...
                    background:
                      ...
                      radial-gradient(circle at 70% 70%, rgba(255,215,0,0.3) 0%, transparent 40%);
                  "
                ></div>
        ...
        <div class="vd-col-12 vd-col-md-6 vd-mb-6">
          <div class="vd-card vd-card-glow demo-card" style="height: 100%">
            <div class="vd-card-header">
              ...
              <p class="vd-text-sm vd-text-muted vd-mb-0">
                <code>detachable: true</code>, <code>minimizable: true</code>, <code>draggable: false</code>. Snap to a corner via <code>floatingPosition</code> or <code>detach(el, position)</code>.
              </p>
            </div>
            <div class="vd-card-body">
              <VdMusicPlayer
              ...
              <div class="vd-mt-4 vd-d-flex vd-gap-2 vd-flex-wrap">
                <button type="button" class="vd-btn vd-btn-primary vd-btn-sm" @click="detachFixed('bottom-left')">
                  <i class="ph ph-arrows-out"></i> Detach bottom-left
                </button>
                <button type="button" class="vd-btn vd-btn-primary vd-btn-sm" @click="detachFixed('top-right')">
                  <i class="ph ph-arrows-out"></i> Detach top-right
                </button>
                <button type="button" class="vd-btn vd-btn-secondary vd-btn-sm" @click="attachFixed()">
        ...
        <div class="vd-col-12 vd-col-md-6 vd-mb-6">
          <div class="vd-card vd-card-glow demo-card" style="height: 100%">
            <div class="vd-card-header">
              ...
              <p class="vd-text-sm vd-text-muted vd-mb-0">
                <code>draggable: true</code> adds a drag handle when floating. Still supports minimize/expand. Free positioning overrides corner presets.
              </p>
            </div>
            <div class="vd-card-body">
              <VdMusicPlayer
              ...
              />
              <div class="vd-mt-4 vd-d-flex vd-gap-2 vd-flex-wrap">
                <button type="button" class="vd-btn vd-btn-primary vd-btn-sm" @click="detachAndMinimize">
                  <i class="ph ph-arrows-out"></i> Detach &amp; minimize
                </button>
                <button type="button" class="vd-btn vd-btn-secondary vd-btn-sm" @click="expandDrag">
                  <i class="ph ph-arrows-in-line-vertical"></i> Expand
                </button>
                <button type="button" class="vd-btn vd-btn-secondary vd-btn-sm" @click="attachDrag">
                  <i class="ph ph-arrows-in"></i> Attach back
                </button>
              </div>
              <DocCodeSnippet
...
VanduoMusicPlayer.attach(el);`"
        ...
        <div class="vd-col-12 vd-col-md-6 vd-mb-6">
          <div class="vd-card vd-card-glow demo-card" style="height: 100%">
            <div class="vd-card-header">
              ...
              <p class="vd-text-sm vd-text-muted vd-mb-0">
                Drive the player through the exposed <code>player</code> instance.
              </p>
            </div>
            <div class="vd-card-body">
              <VdMusicPlayer
              ...
              <div class="vd-d-flex vd-gap-2 vd-flex-wrap">
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progPlay">
                  <i class="ph ph-play"></i> Play
                </button>
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progPause">
                  <i class="ph ph-pause"></i> Pause
                </button>
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progPrev">
                  <i class="ph ph-skip-back"></i> Prev
                </button>
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progNext">
                  <i class="ph ph-skip-forward"></i> Next
                </button>
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progTrack(0)">
                  Track 1
                </button>
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progTrack(2)">
                  Track 3
                </button>
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progVol(0.25)">
                  Vol 25%
                </button>
                <button type="button" class="vd-btn vd-btn-outline vd-btn-sm" @click="progVol(0.75)">
                  Vol 75%
                </button>
              </div>
        ...
        <div class="vd-col-12 vd-col-md-6 vd-mb-6">
          <div class="vd-card vd-card-glow demo-card" style="height: 100%">
            ...
            <div class="vd-card-body">
              <VdMusicPlayer
                :tracks="tracks"
                :options="{ showProgress: true, showPlaylist: true, detachable: true, minimizable: true }"
                @play="onLogged('play')"
              ...
              />
              <div class="vd-d-flex vd-align-items-center vd-justify-content-between vd-mt-4 vd-mb-2">
                <span class="vd-text-sm vd-text-muted">Most recent {{ logEntries.length }} event(s):</span>
                <button type="button" class="vd-btn vd-btn-sm vd-btn-outline" @click="clearLog">Clear</button>
              </div>
              <ul class="vd-mb-0" style="list-style: none; padding: 0; max-height: 180px; overflow-y: auto;">
                <li
                ...
                >
                  <code>{{ e.time }}</code>
                  <span class="vd-badge vd-badge-secondary vd-ml-2">{{ e.type }}</span>
                  <span v-if="e.detail" class="vd-text-muted vd-ml-2">{{ e.detail }}</span>
                </li>
                <li v-if="logEntries.length === 0" class="vd-text-sm vd-text-muted">
      ...
      <div class="vd-card vd-card-glow demo-card">
        ...
        <div class="vd-card-body">
            ...
            <template #vanilla>
              ...
              <div class="vd-table-responsive">
                <table class="vd-table vd-table-striped">
                  ...
                  <tbody>
                    <tr v-for="row in programmaticApi" :key="row[0]"><td><code>{{ row[0] }}</code></td><td>{{ row[1] }}</td></tr>
                  </tbody>
                </table>
              </div>
            </template>
            ...
            <table class="vd-table vd-table-striped">
              ...
              <tbody>
                <tr v-for="row in optionsTable" :key="row[0]">
                  <td><code>{{ row[0] }}</code></td><td>{{ row[1] }}</td><td><code>{{ row[2] }}</code></td><td>{{ row[3] }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <h4 class="vd-mt-6"><i class="ph ph-wheelchair mr-2" style="color: var(--vd-color-primary);"></i>Accessibility</h4>
          <ul>
            <li>The container is a <code>role="region"</code> labeled <strong>Music Player</strong>; transport controls sit in a <code>role="group"</code> labeled <strong>Playback controls</strong>.</li>
            <li>Buttons expose <code>aria-label</code> text (Play/Pause, Previous/Next, Volume, Seek, playlist toggle with <code>aria-expanded</code>, shuffle and repeat with <code>aria-pressed</code>). Repeat uses labels <strong>Repeat</strong>, <strong>Repeat one</strong>, and <strong>Repeat all</strong>; repeat-one shows a visible <code>1</code> badge.</li>
            <li>The current track name uses <code>aria-live="polite"</code> and <code>aria-atomic="true"</code> so screen readers announce track changes.</li>
            <li>Playlist items set <code>aria-current</code> on the active row; decorative icons use <code>aria-hidden="true"</code>.</li>
            <li>Play/Pause supports <kbd>Space</kbd> and <kbd>Enter</kbd> on the play button; range inputs are keyboard-focusable with visible <code>:focus-visible</code> outlines.</li>
            <li>Floating toolbar controls include descriptive <code>aria-label</code> values; minimize toggles <code>aria-expanded</code>.</li>
            <li>Glass and floating transitions respect <code>prefers-reduced-transparency</code> and <code>prefers-reduced-motion</code>.</li>
          </ul>

          <h4 class="vd-mt-6">Attribution</h4>
          <p class="vd-text-sm vd-text-muted vd-mb-0">
            <i class="ph ph-music-notes" style="color:var(--vd-color-primary);margin-right:0.375rem;"></i>
            <strong>Bundled demo audio:</strong> a curated track set from
            <a href="https://stellardrone.bandcamp.com/" target="_blank" rel="noopener noreferrer">Stellardrone</a>
            from the album <em>Invent the Universe</em>. Source:
            <a href="https://stellardrone.bandcamp.com/album/invent-the-universe" target="_blank" rel="noopener noreferrer">Bandcamp</a>.
            Licensed under
