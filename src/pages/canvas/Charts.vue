...
const vue3Api: [string, string][] = [
  ...
  [":theme / :tooltip / :responsive", "Theme overrides, custom/disabled tooltip, responsive resize."],
];
...
<template>
  <DocsLayout>
    <section id="vd-charts">
      ...
      <p class="vd-mb-8">
        <strong>Vanduo Charts</strong> is a standalone SVG-first package for common
        dashboard charts, installed separately from the framework. It renders
        accessible SVG, reads Vanduo theme tokens, and ships an optional Vue 3
        binding (<code>@vanduo-oss/charts/vue</code>) used here.
      </p>

      <div class="vd-row vd-mb-6">
        <div class="vd-col-12 vd-col-lg-4 vd-mb-4">
          <div class="vd-card demo-card">
            <div class="vd-card-header"><h6><i class="ph ph-chart-bar"></i> Bar Chart</h6></div>
            <div class="vd-card-body">
              <VdChart type="bar" :data="barData" x="month" y="sales" title="Monthly sales" :height="300" />
            </div>
          </div>
        </div>
        <div class="vd-col-12 vd-col-lg-4 vd-mb-4">
          <div class="vd-card demo-card">
            <div class="vd-card-header"><h6><i class="ph ph-chart-line-up"></i> Line Chart</h6></div>
            <div class="vd-card-body">
              <VdChart type="line" :data="lineData" x="month" y="visits" title="Site visits" :height="300" />
            </div>
          </div>
        </div>
        <div class="vd-col-12 vd-col-lg-4 vd-mb-4">
          <div class="vd-card demo-card">
            <div class="vd-card-header"><h6><i class="ph ph-chart-donut"></i> Donut Chart</h6></div>
            <div class="vd-card-body">
              <VdChart type="donut" :data="donutData" label="channel" value="revenue" title="Revenue mix" :height="300" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="vd-card-body">
          ...
          <div class="vd-table-responsive">
            <table class="vd-table vd-table-striped">
              ...
              <tbody>
                <tr v-for="row in chartTypes" :key="row[0]"><td><code>{{ row[0] }}</code></td><td>{{ row[1] }}</td></tr>
              </tbody>
            </table>
          </div>
            ...
            <template #vanilla>
              ...
              <div class="vd-table-responsive">
                <table class="vd-table vd-table-striped">
                  ...
                  <tbody>
                    <tr v-for="row in vanillaOptions" :key="row[0]"><td><code>{{ row[0] }}</code></td><td>{{ row[1] }}</td></tr>
                  </tbody>
                </table>
              </div>
            </template>
          ...
          <ul>
            <li>Each chart renders an <code>&lt;svg role="img"&gt;</code> with title/description metadata when provided.</li>
            <li>The vanilla bundle exposes <code>window.VanduoCharts</code> and registers with <code>Vanduo.init(root)</code> when the framework is present.</li>
            <li>Charts read Vanduo theme tokens, so they follow the active theme automatically.</li>
          </ul>
        </div>
      </div>
    </section>
