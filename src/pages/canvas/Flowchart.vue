...
const seedDoc = {
  ...
  nodes: [
    { id: "start", type: "circle", x: 60, y: 140, width: 96, height: 96, text: "Start" },
    { id: "review", type: "rounded-rect", x: 220, y: 140, width: 180, height: 96, text: "Review request" },
    { id: "decision", type: "diamond", x: 470, y: 128, width: 150, height: 120, text: "Approved?" },
    { id: "done", type: "circle", x: 700, y: 140, width: 96, height: 96, text: "Done" },
  ],
  edges: [
    { id: "e1", from: { nodeId: "start", port: "right" }, to: { nodeId: "review", port: "left" }, kind: "arrow", route: "orthogonal" },
    { id: "e2", from: { nodeId: "review", port: "right" }, to: { nodeId: "decision", port: "left" }, kind: "arrow", route: "orthogonal" },
    { id: "e3", from: { nodeId: "decision", port: "right" }, to: { nodeId: "done", port: "left" }, kind: "arrow", route: "orthogonal", label: "yes" },
  ],
};
...
const methods: [string, string][] = [
  ["addNode(node) / updateNode(id, patch) / removeNode(id)", "Add, patch, or delete a node (and its edges)."],
  ["addEdge(edge) / removeEdge(id)", "Connect or disconnect ports; route 'curve' | 'straight' | 'orthogonal'."],
  ["zoomIn() / zoomOut() / fitView() / resetView()", "Adjust viewport scale and framing."],
  ["startTextEdit(id) / stopTextEdit({ commit })", "Inline text editor used by double-click editing."],
  ["toJSON() / load(data) / clear()", "Export, import, or reset the full document."],
  ["on(event, cb) / off(event, cb)", "Subscribe to change, select, viewport, connect."],
];
  ...
  <DocsLayout>
    <section id="vd-flowchart">
      ...
      <p class="vd-mb-8">
        ...
        ports, edit text inline, and export/import JSON. It ships an optional Vue 3
        binding (<code>@vanduo-oss/flowchart/vue</code>) used here.
      </p>

      <div class="vd-card demo-card vd-mb-6">
        <div class="vd-card-header"><h6><i class="ph ph-flow-arrow"></i> Editor</h6></div>
        ...
        <div class="vd-card-body">
          ...
          <div class="vd-table-responsive">
            <table class="vd-table vd-table-striped">
              ...
              <tbody>
                <tr v-for="row in nodeTypes" :key="row[0]"><td><code>{{ row[0] }}</code></td><td>{{ row[1] }}</td></tr>
              </tbody>
            </table>
          </div>
