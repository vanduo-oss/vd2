...
const aspects: [string, string, "success" | "secondary"][] = [
  ["Spacing scale (--vd-space-fib-*)", "Fibonacci — 1, 2, 3, 5, 8, 13, 21…", "success"],
  ["Radius scale", "Fibonacci", "success"],
  ["Shadows & elevation", "Fibonacci offset/blur pairs (1/2, 2/3, 3/5…)", "success"],
  ...
  ["Golden & Fibonacci grids", "Opt-in (.vd-grid-golden, .vd-row-fib-*)", "success"],
  ["Default 12-column grid", "Equal columns — φ grids are a class away", "secondary"],
  ["Responsive breakpoints", "Familiar 576 / 768 / 992 / 1200 / 1400", "secondary"],
  ["z-index & transition timings", "Standard increments", "secondary"],
];
const aspectBadge = (v: "success" | "secondary"): string => (v === "success" ? "φ" : "conventional");
</script>
    ...
    <section id="golden-ratio">
      ...
      <div class="vd-row">
        <div class="vd-col-12">
          <div class="vd-card vd-card-glow demo-card vd-mb-8">
            <div class="vd-card-header"><h6>Golden Split (38.2% / 61.8%)</h6></div>
      ...
      <div class="vd-row">
        <div class="vd-col-12 vd-col-md-6">
          <div class="vd-card vd-card-glow demo-card vd-mb-8">
            <div class="vd-card-header"><h6>Fibonacci 3-Column (2:3:5)</h6></div>
            <div class="vd-card-body">
              <div class="vd-row-fib-3">
                <div class="vd-col-fib-2">
                  <div class="vd-bg-info vd-text-white vd-p-5 vd-rounded-md vd-text-center vd-text-sm">2</div>
                </div>
                <div class="vd-col-fib-3">
                  <div class="vd-bg-success vd-text-white vd-p-5 vd-rounded-md vd-text-center vd-text-sm">3</div>
                </div>
                <div class="vd-col-fib-5">
                  <div class="vd-bg-primary vd-text-white vd-p-5 vd-rounded-md vd-text-center vd-text-sm">5</div>
                </div>
              </div>
        ...
        <div class="vd-col-12 vd-col-md-6">
          <div class="vd-card vd-card-glow demo-card vd-mb-8">
            ...
            <div class="vd-card-body">
              <div class="vd-grid-golden">
                <div class="vd-bg-warning vd-p-5 vd-rounded-md vd-text-center vd-text-sm">1fr</div>
                <div class="vd-bg-primary vd-text-white vd-p-5 vd-rounded-md vd-text-center vd-text-sm">1.618fr</div>
              </div>
          ...
          <div class="vd-card vd-card-glow demo-card">
            ...
            <div class="vd-card-body">
              <div class="vd-d-flex vd-flex-col vd-gap-2">
                <div v-for="n in fibScale" :key="n" class="vd-d-flex items-center vd-gap-5">
                  <code class="vd-text-sm">{{ n }}px</code>
                  <div class="vd-bg-primary" :style="`width: ${n}px; height: 13px;`"></div>
                </div>
              </div>
      ...
      <div class="vd-row">
        <div class="vd-col-12">
          <div class="vd-card vd-card-glow demo-card vd-mb-8">
            <div class="vd-card-header"><h6>Feel the ratio — interactive spacing scale</h6></div>
            <div class="vd-card-body">
              <p class="vd-mb-5">
                Drag through the Fibonacci spacing scale. Each step's ratio to the
                one before it sits near <strong>φ (1.618)</strong> — that
                convergence is why the rhythm feels balanced without manual tuning.
              </p>
              <div>
                <input
                ...
                />
                <div class="vd-mt-6" style="display: flex; align-items: center; gap: 1.25rem">
                  <div :style="`height: 28px; width: ${fibPx}px; background: var(--vd-color-primary); border-radius: var(--vd-radius-fib-3); transition: width 0.15s ease;`"></div>
                  <div>
      ...
      <div class="vd-row">
        <div class="vd-col-12">
          <div class="vd-card vd-card-glow demo-card vd-mb-8">
            <div class="vd-card-header"><h6>Where the golden ratio lives — and where it doesn't</h6></div>
            <div class="vd-card-body">
              <p class="vd-mb-5">
              ...
              <div class="vd-table-responsive">
                <table class="vd-table vd-table-hover">
                  <thead>
                    <tr><th style="width: 45%">Aspect</th><th style="width: 40%">Basis</th><th style="width: 15%"></th></tr>
                  </thead>
                  <tbody>
                    <tr v-for="row in aspects" :key="row[0]">
                      <td>{{ row[0] }}</td>
                      <td>{{ row[1] }}</td>
                      <td><span class="vd-badge" :class="`vd-badge-${row[2]}`">{{ aspectBadge(row[2]) }}</span></td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <p class="vd-text-sm vd-text-muted vd-mt-5">
                ...
                <RouterLink to="/core/grid-system">Grid System</RouterLink> page.
              </p>
            </div>
          </div>
        </div>
