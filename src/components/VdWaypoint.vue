...
const setActive = (id: string | null): void => {
  ...
  const nav =
    (props.navSelector
      ? root.value.querySelector(props.navSelector)
...
const setupObserver = (): void => {
  ...
  if (!root.value) return;
  const el = root.value;
  const sections = Array.from(el.querySelectorAll<HTMLElement>("[id]")).filter(
    (section) =>
      el.contains(section) || section.closest(el.tagName.toLowerCase()) === el,
  );
